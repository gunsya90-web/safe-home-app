...
(function (SAFEHOME) {
  ...
  function render() {
    ...
    root.innerHTML =
      ...
      '<div class="dash-grid-2 ff-grid-detail">' +
        '<div class="panel">' +
          '<h3 class="panel-title">🗺️ 실시간 세대 현황 <span class="badge">' + activeCount + '</span></h3>' +
          '<div class="ff-grid-lg">' + SAFEHOME.renderOccupancyGrid(units, { selected: selectedHo }) + '</div>' +
  ...
  function evacuationPanelHtml(afp) {
    ...
    return SAFEHOME.detailsPanel('🚪 피난시설 현황 (AFP-Core)', body, { id: 'evacuation', openState: panelOpenState.evacuation });
  }
  ...
  function unitDetailHtml(ho, incident, building) {
    if (!ho) {
      return '<h3 class="panel-title">🧾 세대 상세</h3><div class="empty-note">왼쪽 현황판에서 세대를 선택하세요.</div>';
    }
    var u = incident.units[ho];
    if (!u) return '<h3 class="panel-title">🧾 세대 상세</h3><div class="empty-note">세대 정보를 찾을 수 없습니다.</div>';
    ...
    return '<h3 class="panel-title">🧾 ' + (u.isFireOrigin ? '🔥 ' : '') + ho + '호' + (isDuplex ? ' 🪜' : '') + ' 상세 <span style="color:' + meta.color + ';font-size:12.5px;">' + meta.label + '</span></h3>' +
